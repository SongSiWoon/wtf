...
FileManager::FileManager(QObject* parent, CMModelAgent* aAgent)
    : QObject(parent), _agent(aAgent)
    ...
    , _systemIdQGC(255)
{
    _ackTimer = new QTimer(this);
    connect(_ackTimer, SIGNAL(timeout()), this, SLOT(_ackTimeout()));
...
{
    qDebug("_closeUploadSession : %d", success);
...
//        Q_EMIT commandProgress(100 * ((float)(_readFileAccumulator.length() - _missingDownloadedBytes) / (float)_downloadFileSize));
//    }
...
void FileManager::_writeAckResponse(Request* writeAck)
{
    if(_writeOffset + _writeSize >= _writeFileSize){
        SendSCDialog::ftpResult(_agent->id(), OperationState::kCOUpload, true);
        _closeUploadSession(true /* success */);
        return;
    }

    if (writeAck->hdr.session != _activeSession) {
        SendSCDialog::ftpResult(_agent->id(), OperationState::kCOUpload, false);
    ...
    if (incomingSeqNumber != expectedSeqNumber) {
        ...
        switch (_currentOperation) {
            case kCOBurst: // burst download drops are handled in _downloadAckResponse()
                doAbort = false;
                SendSCDialog::ftpResult(_agent->id(), _currentOperation, false);
                break;
            case kCORead:
                _closeDownloadSession(false /* failure */);
                SendSCDialog::ftpResult(_agent->id(), _currentOperation, false);
...
//        qDebug("request->hdr.req_opcode : %d", request->hdr.req_opcode);
//        qDebug("_currentOperation : %d", _currentOperation);
        switch (request->hdr.req_opcode) {
			case kCmdListDirectory:
                _listAckResponse(request);
                SendSCDialog::ftpResult(_agent->id(), OperationState::kCOList, true);
                break;
			case kCmdOpenFileRO:
            case kCmdOpenFileWO:
                _openAckResponse(request);
                SendSCDialog::ftpResult(_agent->id(), _currentOperation, true);
                break;
            ...
            case kCmdWriteFile:
                SendSCDialog::ftpResultWrite(_agent->id(), _currentOperation, true, _writeOffset + _writeSize, _writeFileSize);
//                _writeOffset + _writeSize >= _writeFileSize
...
void FileManager::removeDirectory(const QString& directory)
{
    if(_currentOperation != kCOIdle){
        _emitErrorMessage(tr("UAS File manager busy. Try again later"));
        return;
    ...
    _currentOperation = kCORemoveDir;

    Request request;
    request.hdr.session = 0;
    request.hdr.opcode = kCmdRemoveDirectory;
    request.hdr.offset = 0;
    request.hdr.size = 0;
    _fillRequestWithString(&request, directory);
    _sendRequest(&request);
...
void FileManager::removeFile(const QString &file_path)
{
    if(_currentOperation != kCOIdle){
        _emitErrorMessage(tr("UAS File manager busy. Try again later"));
        return;
    ...
    _currentOperation = kCORemoveFile;
    ...
    request.hdr.offset = 0;
    request.hdr.size = 0;
    _fillRequestWithString(&request, file_path);
    _sendRequest(&request);
...
//    _ackTimer->start(ackTimerTimeoutMsecs);
    ...
    QMetaObject::invokeMethod(_ackTimer, "start", Qt::QueuedConnection, Q_ARG(int, 500));
}

/// @brief Clears the ac￩￣￩k timeout timer
...
//        qCDebug(FileManagerLog) << "ack timeout - retrying";
    ...
    switch (_currentOperation) {
        ...
        default:
        {
            ...
            SendSCDialog::ftpResult(_agent->id(), currentOperation, false);
        }
            break;
    }
}
...
{
    qDebug("_emitErrorMessage: %s", msg.toLatin1().data());

    Q_EMIT commandError(msg);
}

void FileManager::_emitListEntry(const QString& entry)
{
//    qDebug("_emitListEntry: %s", entry.toLatin1().data());
    SendSCDialog::ftpResultList(_agent->id(), entry);

    QStringList entryDataList = entry.mid(1,entry.count() - 1).split("\t");
    if (entry.at(0) == 'F') {
        QString fileName = entryDataList.at(0);
        QString fileSize = entryDataList.at(1);
        _agent->data("DIR_FILES").toStringList().push_back(fileName + "," + fileSize);
    } else {
        QString dirName = entryDataList.at(0);
        _agent->data("DIR_FILES").toStringList().push_back(dirName);
    }
    Q_EMIT listEntry(entry);
}
...
void FileManager::_sendRequest(Request* request)
{
    ...
    if (request->hdr.size <= sizeof(request->data)) {
        ...
        qDebug("ERROR: request length too long: %d", request->hdr.size);
    }
...
void FileManager::_sendRequestNoAck(Request* request)
{
    ...
    mavlink_msg_file_transfer_protocol_pack_chan(_systemIdQGC,       // QGC System ID
                                                 0,                  // QGC Component ID
                                                 0,//link->mavlinkChannel(),
                                                 &message,           // Mavlink Message to pack into
                                                 0,                  // Target network
                                                 _agent->sysID(),    // Target system
                                                 0,                  // Target component
                                                 (uint8_t*)request); // Payload
    _agent->sender()->transmit(message);
}
